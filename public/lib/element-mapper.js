...
// Which slot each composite reads its content from.
//
// MEASURED, not read off the source — every type fed both slots, with both
// payload shapes (link-ish `{title, link}` and element specs `{type, text}`),
// in a real browser. Reading it off the source is how this got written down
// wrong three times, most recently `wrap`, which was recorded as accepting
// either and in fact reads `children` alone.
//
// The shapes differ per type and are deliberately NOT encoded here: `cards`,
// `nav` and `sideNav` take link-ish items, `ulist` takes element specs,
// `table` takes either. This map answers only "which slot", which is the
// question the silent failure below turns on.
//
// `grid` is absent because it is not a mapper type at all: it is a legacy
// branch in designer.js that emits a stored template and honours neither slot.
// It is unreachable from the schema, so no agent can write it and there is no
// correct slot to point one at.
//
...
function warnWrongSlot(el) {
    const want = CONTENT_SLOT[el && el.type];
    if (!want) return;
    const other = want === "items" ? "children" : "items";
    const has = (k) => Array.isArray(el[k]) && el[k].length > 0;
    if (has(other) && !has(want)) {
        const where = el.id ? ` (id "${el.id}")` : "";
        console.warn(
            `[nodality] "${el.type}"${where} carries content in "${other}" but reads ` +
            `"${want}". The "${other}" content is ignored and the placeholder template ` +
            `rendered instead. Move it to "${want}".`,
        );
    }
}
    ...
    static input(obj){
      ...
      static picker(obj){
        ...
        return new Picker()
          .set({
              items: Array.isArray(el.items) ? el.items : [],
              // `pad`, not the component's private `arrayPadding`, which is
              // deprecated. Identical output — both emit `padding: 0.5rem` —
              // and it stops every generated picker warning about an option
