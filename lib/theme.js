...
const Theme = {
    ...
    // .theme({...}) map or an inline color. The LIGHT default is black (#111)
    // so text renders black out of the box without each project calling
    // setDefaults() — components that set their own color inline always win
    // (see _applyTheme: it never overwrites a user-set key).
    //
    // DARK is intentionally left EMPTY: setting a built-in dark color would
    // turn text white for visitors whose OS prefers-color-scheme is dark
    // (getInitialMode() → "dark"), making it invisible on light-background
    // sites that don't implement a dark background. Sites that DO implement
    // dark mode opt in per-project via Theme.setDefaults({ dark: {...} }).
    //
    // Per-component theming via .set({ theme: {...} }) works regardless.
    defaults: {
        light: { color: "#111" },
        dark:  {}
    },
