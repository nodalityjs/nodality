...
new Text("row.").set({
    ...
    add(arr) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            let ops = ["blast", "gradient", "shadow", "filter", "animationSimple", "transform", "span"];
...
new Text("One").set({fluidc: "S1", color: "#1abc9c", font: "Arial"}),
  ...
  new Slider(texts, {
    ...
    url: "true",
      img: new Image().set({ url: "../assets/arrow-left.png", width: "50px", height: "50px"}),
      color: "#1abc9c", // ORDER SWITCH?
     // frame: { width: 50, height: 50 },
      radius: "100%",
      arrayMargin: { sides: ["all"], value: "1rem" }
      // 10/01/2025 18:12:55 margin does not add to page width, padding does
      // Attr <a href="https://www.flaticon.com/free-icons/back" title="back icons">
      // Back icons created by Lyolya - Flaticon
    }),

    rightButton: new Button("").set({
      url: "true",
        img: new Image().set({ url: "../assets/arrow-right.png", width: "50px", height: "50px"}),
      color: "#1abc9c",
      frame: { width: 50, height: 50 },
      radius: "100%",
      arrayMargin: { sides: ["all"], value: "1rem" }
