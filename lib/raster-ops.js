            ...
            params: {
                count: {
                    ...
                    summary: "stamps arranged in a ring around the focus. The loop is "
                        ...
                        + "This default is the shader's; the DOM path that an element of type "
                        + "\"copy\" takes uses 3 instead.",
                },
                points: {
                ...
                animation: {
                    ...
                    // Read by ElementMapper.mapCopy, not by this file. The
                    // doc-consistency check scans raster-ops.js for
                    // `node.<key>` and would otherwise call this a param that
                    // nothing reads; naming the reader moves that check to the
                    // right source instead of switching it off.
                    readBy: "element-mapper",
                    summary: "DOM path only. An element of type \"copy\" is built by the "
                        + "mapper rather than by the shader, and this asks that mapper for "
                        + "animated copies; the shader path ignores it. Declared because "
                        + "the mapper has always read it and the docs have always taught "
                        + "it, while the registry did not list it – so the validator "
                        + "reported a documented, working node as an unknown parameter.",
                },
            },
        },
...
function applyRasterPipeline(el, rasterNodes, opts) {
    ...
    const syncTransitionView = () => {
        ...
        // Ink follows the canvas, at BOTH ends. Restoring only at `done`
        // was correct while the only element that presented at t=0 lived
        // outside this host — a single morph keeps its source in a
        // separate layer, so nothing inside ever needed its ink back at
        // zero. In a chain the state returned to IS inside the host, and
        // it presented with the ink still suppressed: text laid out,
        // selectable, and completely invisible until dragged over, since
        // the selection rule is the one thing that repaints it.
        if (done || atStart) {
