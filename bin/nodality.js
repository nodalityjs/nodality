...
// ─── stage subcommand ────────────────────────────────────────────
//
// One-liner that consumer `upgrade.sh` scripts can call instead of
// hand-rolling:
//
//   cp node_modules/nodality/dist/index.esm.js upload/dist/lib.bundle.js
//   node -e "stamp version into upload/_nodality-version.js"
//
// Both bits of bookkeeping run in EVERY consumer's upgrade flow and
// were duplicated nearly verbatim across h7-nodality, sls3-2025 and
// sls3-2026. Now a single `nodality stage` covers them — future
// consumers get it for free, and version drift across copies is
// impossible.
//
// Flags:
//   --upload=<dir>     consumer's upload/ directory (default ./upload)
...
async function runStage(args) {
  ...
  const cwd = process.cwd();
  const uploadDir = path.resolve(cwd, flags.upload ?? "./upload");

  if (!fs.existsSync(uploadDir)) {
    console.error(`[nodality] stage: upload dir not found: ${uploadDir}`);
    process.exit(1);
  }

  // Locate the installed nodality so we copy from the same version
  // the consumer's `npm install` just landed. Walks up from `cwd`
  // looking for node_modules/nodality.
  let pkgRoot = null;
  let cursor = cwd;
  while (cursor !== path.dirname(cursor)) {
    const candidate = path.join(cursor, "node_modules", "nodality");
  ...
  if (!pkgRoot) {
    console.error(`[nodality] stage: could not locate node_modules/nodality from ${cwd}`);
    process.exit(1);
  }
  ...
  if (flags["no-bundle"] !== true) {
    const src = path.join(pkgRoot, "dist", "index.esm.js");
    const dst = path.join(uploadDir, "dist", "lib.bundle.js");
  ...
  // 2. Version stamp — surfaced in consumer footers ("nodality vX.Y.Z")
  //    so a deployed page proves which engine version it was built
  //    against. Idempotent: only writes if the version actually
  //    changed, so re-running doesn't churn the file's mtime.
  if (flags["no-version"] !== true) {
    const stampPath = path.join(uploadDir, "_nodality-version.js");
    const body =
      `// Auto-generated by \`nodality stage\` — do not hand-edit.\n` +
      `// Reflects the version of the \`nodality\` package currently\n` +
      `// installed in node_modules/, so the footer can display a\n` +
      `// tiny build tag without bundling node_modules into the\n` +
      `// static site.\n` +
      `export const NODALITY_VERSION = "${version}";\n`;
    let needsWrite = true;
    if (fs.existsSync(stampPath)) {
      const existing = fs.readFileSync(stampPath, "utf8");
      needsWrite = existing !== body;
    }
    if (needsWrite) {
      fs.writeFileSync(stampPath, body);
      console.log(`[nodality] stage: version ${version} → ${path.relative(cwd, stampPath)}`);
    } else {
